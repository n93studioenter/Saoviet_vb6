...
Begin VB.Form FrmThanhPham 
   ...
   Begin VB.CheckBox Chk 
      BackColor       =   &H00FFFFFF&
      Caption         =   "Tù ®éng xuÊt NVL tõ kho"
      Height          =   255
      Left            =   4680
      TabIndex        =   6
   ...
   Begin VB.TextBox txtChuyen 
      ...
      Index           =   0
      Left            =   120
      LinkItem        =   "Sè hiÖu vËt t­, thµnh phÈm (nhÊn ENTER ®Ó xem danh s¸ch)"
      MaxLength       =   20
      TabIndex        =   14
      Top             =   6240
      Width           =   1335
   ...
   End
End
Attribute VB_Name = "FrmThanhPham"
...
End Sub
'====================================================================================================
' HiÓn thÞ phiÕu ®­îc chän
'====================================================================================================
Private Sub CboSohieu_Click()
    Dim rs As Recordset, i As Integer, tt As Double, dgia As Double, k As Integer
    Dim nvl As Double, nC As Double, sxc As Double
    
    If CboSohieu.ListIndex < 0 Or flag = 1 Then Exit Sub
    flag = 1
    k = CboSohieu.ListIndex
    ClearGrid Grd, Grd.tag
    ...
    Do While Not rs.EOF
        ...
        Grd.AddItem rs!shtk + Chr(9) + rs!shvt + Chr(9) + rs!TenVattu + Chr(9) + rs!DonVi + Chr(9) + Format(rs!SoLuong, Mask_2) + Chr(9) + Format(dgia, Mask_2) + Chr(9) + Format(tt, Mask_0) + Chr(9) + CStr(rs!MaTK) + Chr(9) + CStr(rs!MaTP) + Chr(9) + DoiDau(rs!CPNVL) + Chr(9) + DoiDau(rs!cpnc) + Chr(9) + DoiDau(rs!cpsxc), 0
        nvl = nvl + rs!CPNVL
        nC = nC + rs!cpnc
        sxc = sxc + rs!cpsxc
        rs.MoveNext
    ...
    If (Shift And vbAltMask) > 0 Then
        Select Case KeyCode
            ...
            Case vbKeyV:
                ...
                Command_Click 3
            Case vbKeyR:
                If pGiaHT > 0 Then TinhLaiGiaThanh
        End Select
    End If
...
End Sub
'====================================================================================================
' Thñ tôc kiÓm tra phiÕu l­u chuyÓn cã hîp lÖ ?
'====================================================================================================
...
End Function
'======================================================================================
' Thñ tôc xãa phiÕu trªn mµn h×nh
'======================================================================================
    ...
    With Grd
        ...
        If r >= 0 And tcp <> 0 Then
            For i = 0 To r - 1
                ...
                .col = 6
                .Text = Format(Cdbl5(.Text) + cp1, Mask_0)
                .col = 9
                .Text = Format(cp1, Mask_0)
                CP = CP + cp1
            Next
            .Row = r
            .col = 6
            .Text = Format(Cdbl5(.Text) + tcp - CP, Mask_0)
            .col = 9
            .Text = Format(Cdbl5(.Text) + tcp - CP, Mask_0)
        End If
...
Private Sub InGiaThanh()
    ...
    frmMain.Rpt.WindowTitle = "Gi¸ thµnh thµnh phÈm hoµn thµnh trong kú"
...
Private Sub InNVL()
    ...
    If m.MaSo > 0 Then frmMain.Rpt.Formulas(12) = "DG='" + m.sohieu + " - " + m.TenVattu + "'"
    frmMain.Rpt.WindowTitle = "Chi tiÕt chi phÝ nguyªn vËt liÖu trùc tiÕp"
    frmMain.Rpt.Destination = crptToWindow
    Me.MousePointer = 0
    InBaoCaoRPT
End Sub
...
Private Sub TinhLaiGiaThanh()
    Dim i As Integer
    Me.MousePointer = 11
    ktra = 1
    For i = 0 To CboSohieu.ListCount - 1
        CboSohieu.ListIndex = i
        Command_Click 1
    Next
    ktra = 0
    KiemTraVatTu
